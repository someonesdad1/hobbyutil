...
Aug 2026 update:  I will use this file to 
    - Produce the README.md file
    - Check that the requisite files are present
    ...
    Projects = {
        ...
        'triguc': {
            ...
            'descr': dedent('''
                ...
                circle.  I don't include the python script to make it anymore.
                '''),
            'files': [
                    #'trig_functions.py',
...
if 1:   # Core functionality
    def CheckFiles(di, pr):
        ...
        for item in files:
            srcdir = Path(di["srcdir"])
            # Make sure the source file exists
            if isinstance(item, str):
                # This must be an existing file
                p = srcdir/Path(TrimAsterisk(item))
            else:
                ...
                if len(item) != 2 or not all(isinstance(i, str) for i in item):
                    Error(f"{item!r} is an improper sequence")
                first, second = item
                p = srcdir/Path(TrimAsterisk(first))
    ...
    def Validate(warn=False):
        ...
        for project in Projects:
            ...
            if "todo" in di and not isinstance(di["todo"], str):
                Error(f"'todo' in project {pr} must be a string")
            if "ignore" in di and not isinstance(di["ignore"], str):
    ...
    def UpdateFile(real_name, path):
        '''Check if the file at path exists; if not, copy real_name to path.  Otherwise, 
        hash the two files and copy real_name to path if the hashes don't match.
        '''
        if not path.exists():
            # Copy real_name to path
            shutil.copy(real_name, path)
            Dbg(f"{path} doesn't exist; copied")
        else:
    ...
    def ProcessProject(name):
        ...
        is a dict that has 
            - One PDF file name (example 'bnc')
                - This is copied to the relevant directory
            - One entry that's a list (example 'triguc')
                - [a, b] where a is the actual file and b is the resulting name]
        ...
        if len(files) == 1: # A PDF or a list
            if isinstance(files[0], list):
                ...
                real_name = Path(srcdir)/real_name
                Dbg(f"  Real name of file is {str(real_name)!r}")
